...
if [[ "${ROOTLESS}" == "true" ]]; then
  ...
  if [[ -f /etc/debian_version ]]; then
    ...
    echo "If this build fails, rerun with: PODMAN_USE_SUDO=1 ./scripts/build-iso.sh" >&2
  fi
else
...
echo "Building ISO in ${OUTPUT_DIR} using ${BUILD_CONFIG}"
"${PODMAN_CMD[@]}" "${PODMAN_ARGS[@]}" \
  "${BUILDER_IMAGE}" \
  "${BUILD_ARGS[@]}" \
  "${IMAGE_NAME}"

echo "ISO image build complete"
find "${OUTPUT_DIR}" -type f -name '*.iso' | sort
